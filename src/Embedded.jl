...
function _cut_sm(model::DiscreteModel,geom::STLGeometry)
  ...
  out = get_subcell_coords(subgrid,get_grid(model),cell_to_bgcell)
  cell_to_points,point_to_coords,point_to_rcoords = out 
  ...
  out = get_subcell_coords(get_grid(_stl),get_grid(model),f_to_bgcell)
  f_to_points,point_to_coords,point_to_rcoords = out 
...
function get_subcell_coords(subgrid,grid,cell_to_bgcell)
  ...
  for cell in 1:num_cells(subgrid)
    ...
    push!(cell_to_points,points)
    append!(point_to_coords,coords)
    append!(point_to_rcoords,rcoords)
  end
  cell_to_points,point_to_coords,point_to_rcoords
end
