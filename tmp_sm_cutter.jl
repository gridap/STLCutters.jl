...
# get_face, num_faces (box)

# struct square: distance,etc
...
function num_faces_from_vertex(::CartesianMesh{D},d::Integer) where D
  factorial(D) / ( factorial(d) * factorial(D-d) )
end

## TODO: complete CartesianMesh ( use memory or not?? )
## cell ↦ faces
## dfaces ↦ nfaces 
## faces ↦ coordinates

function get_face_id(m::CartesianMesh,vertex::Integer, i::Integer)
  num_vertices(m) * (i-1) + vertex
end


function get_edge(m::CartesianMesh,vertex::Integer,i::Integer)
  get_face(m,Val{1}(),vertex,i)
end

function get_face(m::CartesianMesh,::Val{1},vertex::Integer,i::Integer)
  p1_coord = cartesian_vertex_coordinates(m,vertex)
  p2_coord = p1_coord .+ canonical_vector( Val{D}(), i )
  p1 = get_vertex_coordinates(m,p1_coord)
  p2 = get_vertex_coordinates(m,p2_coord)
  Segment( p1, p2 )
end

function get_face(m::CartesianMesh,::Val{2},vertex::Integer,i::Integer)
end
