...
import STLCutter: closest_point, distance
...
function cells_containg_nfaces!(
  ...
  sm_face::Integer,
  n::Integer,
  sm_vertex_to_d::Vector,
  sm_vertex_to_dface::Vector)
  ...
  for lnface in 1:length( sm_df_to_sm_nf, sm_d )
    ...
    for lvertex in 1:length( sm_nf_to_sm_v, sm_nface )
      ...
      dface = sm_vertex_to_dface[ sm_vertex ]
      d = sm_vertex_to_d[ sm_vertex ] 
      if dface != UNSET
...
function fetch_boundary_nfaces!(
  nfaces::Vector,
  d_to_dface_to_vertices::Vector{<:Table},n::Integer,
  m::VolumeMesh,bg_cell::Integer,
  vertex_to_bg_d::Vector,vertex_to_bg_dface::Vector)

  resize!(nfaces,0)
  nf_to_v = d_to_dface_to_vertices[n+1]
  for nface in 1:length(nf_to_v)
    if is_nface_in_bg_cell(nf_to_v,nface,m,bg_cell,vertex_to_bg_d,vertex_to_bg_dface)
      push!(nfaces,nface)
    end
  end
  nfaces
...
function is_nface_in_bg_cell(
  nface_to_vertices::Table,nface::Integer,
  m::VolumeMesh,bg_cell::Integer,
  vertex_to_bg_d::Vector,
  vertex_to_bg_dface::Vector)

  D = num_dims(m)
  for lvertex in 1:length(nface_to_vertices,nface)
    vertex = nface_to_vertices[nface,lvertex]
    bg_d = vertex_to_bg_d[vertex]
    bg_dface = vertex_to_bg_dface[vertex]
...
for d in 1:D-1
  for sm_dface in 1:num_faces(sm,d)
    n = d-1
    cells_containg_nfaces!(
      bg_cells,vm,
      sm,d,sm_dface,n,
      vertex_to_bg_d,vertex_to_bg_dface)
    @show bg_cells
    while length(bg_cells) > 0
      bg_cell = pop!(bg_cells)
      fetch_boundary_nfaces!(
        nfaces,d_to_dface_to_vertices,n,vm,bg_cell,vertex_to_bg_d,vertex_to_bg_dface)
      @show nfaces
    end
  end
end
